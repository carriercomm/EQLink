...
class eq_search
{
	...
	function parsesearch($table, $return_fields, $search_fields, $weights, $handler, $no_results, $where, $order) {
		global $eqSql, $query, $tp, $search_prefs, $pre_title, $search_chars, $search_res, $result_flag;
		...
		if ($ps['results'] = $eqSql->eqdb_Query($sql_query,"EQSearch")) {
			if (!$search_prefs['mysql_sort']) {
				$x = 0;
				foreach ($search_fields as $field_key => $field) {
		...
		if ($search_prefs['mysql_sort']) {
			$eqSql -> eqdb_Query("SELECT FOUND_ROWS()");
			$frows = $eqSql -> eqdb_Fetch();
			$ps['results'] = $frows[0];
		}
		return $ps;
	}
